...
func genCallbackNum(regReqMsg *message.Message) string {
...
func checkRegisterData(regReqMsg *message.Message) byte {
...
func registerTBox(regReqMsg *message.Message, bid, eventCreationTime uint32, newAesKey string) error {
	tboxDB, err := database.GetDB("iovdb")
	if err != nil {
		logger.Error(err)
		return err
...
func getDispatchData(regReqMsg *message.Message, encryptServData []byte, result byte) ([]byte, error) {
...
func getMessageHeaderData(serviceData []byte, bid uint32) ([]byte, error) {
...
func RegisterACK(conn message.MessageConn, regReqMsg *message.Message) error {
	msg := message.Message{
		Connection: conn,
	}
	...
	//Check data validity
	result = checkRegisterData(regReqMsg)
	if result == RegisterSuccess || result == AlreadyRegister {
		callbackNum = genCallbackNum(regReqMsg)
		bid = util.GenRandUint32()

		err := registerTBox(regReqMsg, bid, regReqMsg.DisPatch.EventCreationTime, callbackNum)
		if err != nil {
			logger.Error(err)
			return err
		}
	...
	dispatchData, err := getDispatchData(regReqMsg, encryptServData, result)
	if err != nil {
		logger.Error(err)
		return err
	}

	messageHeaderData, err := getMessageHeaderData(serviceData, bid)
	if err != nil {
		logger.Error(err)
		return err
	}

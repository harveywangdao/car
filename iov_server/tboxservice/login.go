...
func (login *Login) LoginRequest(eve *Event, reqMsg *message.Message) error {
	...
	eve.PushEventChannel(EventLoginChallenge, reqMsg)
	return nil
}

func (login *Login) LoginChallenge(eve *Event, reqMsg *message.Message) error {
	...
	/*	aesKey, err := login.GetAesKey(respMsg.MesHeader.Bid, login.loginReqServData.KeyType)
		if err != nil {
			logger.Error(err)
			return err
		}
...
func (login *Login) LoginResponse(eve *Event, respMsg *message.Message) error {
	if login.loginStatus != LoginChallengeStatus {
		logger.Error("Need LoginChallenge!")
		eve.PushEventChannel(EventLoginFailure, respMsg)
	...
	if login.loginRespServData.AccessKey == login.genMd5Abstract16Bytes(login.loginChallServData.PlatRandom+key) {
		eve.PushEventChannel(EventLoginSuccess, respMsg)
	} else {
		eve.PushEventChannel(EventLoginFailure, respMsg)
	}
...
func (login *Login) LoginFailure(eve *Event, respMsg *message.Message) error {
	...
	if eve.checkAesKeyOutOfDate(respMsg.MesHeader.Bid) {
	...
	if login.loginStatus == LoginResponseStatus {
		...
		aesKey, err = login.GetAesKey(respMsg.MesHeader.Bid, KeyTypePreAesKey)
		if err != nil {
			logger.Error(err)
			return err
		}
...
func (login *Login) LoginSuccess(eve *Event, respMsg *message.Message) error {
	if login.loginStatus != LoginResponseStatus {
		logger.Error("Need LoginResponse!")
		return errors.New("Need LoginResponse!")
	}
	...
	err = eve.SetBid(respMsg.MesHeader.Bid)
	if err != nil {
		logger.Error(err)
		return err
	}
...
func (login *Login) GetAesKey(bid uint32, keyType uint8) (string, error) {
	tboxDB, err := database.GetDB(DBName)
	if err != nil {
		logger.Error(err)
		return "", err
